...
Program-level production readiness across the full VM lifecycle. The real risk is that teams usually notice weak VM design only after a deployment freeze, region capacity issue, security review, or performance incident.

**Real-world scenario**: A platform team standardizes VM patterns for shared enterprise landing zones and needs a single review path before go-live.
...
<!-- diagram-id: best-practices-index-why-this-matters -->
```mermaid
flowchart TD
    A[Production review workflow] --> B[Workload classification]
...
### CLI example: best practices review
