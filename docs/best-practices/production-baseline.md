...
content_sources:
  diagrams:
  - id: best-practices-production-baseline-why-this-matters
    type: flowchart
...
<!-- diagram-id: best-practices-production-baseline-why-this-matters -->
```mermaid
flowchart TD
...
| `--set` | Set the review-area and owner tags (reviewArea=baseline-validation, owner=platform-team). |
| `--output` | Output format for the response (JSON here). |
