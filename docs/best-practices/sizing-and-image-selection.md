...
content_sources:
  diagrams:
  - id: best-practices-sizing-and-image-selection-why-this-matters
    type: flowchart
...
Workload-based VM size choices, image governance, and performance trade-offs. The real risk is that teams usually notice weak VM design only after a deployment freeze, region capacity issue, security review, or performance incident.

**Real-world scenario**: A digital commerce team must choose sizes for API, SQL relay, and jump-host workloads while keeping regional quotas and cost envelopes under control.
...
<!-- diagram-id: best-practices-sizing-and-image-selection-why-this-matters -->
```mermaid
flowchart TD
    A[Sizing decision path] --> B[Workload classification]
...
### CLI example: sizing and image selection review
...
az vm update     --resource-group $RG     --name $VM_NAME     --set tags.reviewArea=size-image-selection tags.owner=platform-team     --output json
```
...
| `--set` | Set the review-area and owner tags (reviewArea=size-image-selection, owner=platform-team). |
| `--output` | Output format for the response (JSON here). |
