...
content_sources:
  diagrams:
  - id: best-practices-security-best-practices-why-this-matters
    type: flowchart
...
Identity-first administration, Trusted Launch, segmentation, encryption, and just-in-time access. The real risk is that teams usually notice weak VM design only after a deployment freeze, region capacity issue, security review, or performance incident.

**Real-world scenario**: A regulated workload must satisfy internal audit controls for privileged access, segmentation, tamper-resistant boot, and data protection.
...
<!-- diagram-id: best-practices-security-best-practices-why-this-matters -->
```mermaid
flowchart TD
    A[Defense in depth for Azure VMs] --> B[Workload classification]
...
### CLI example: security best practices review
...
az vm update     --resource-group $RG     --name $VM_NAME     --set tags.reviewArea=security-hardening tags.owner=platform-team     --output json
```
...
| `--set` | Set the review-area and owner tags (reviewArea=security-hardening, owner=platform-team). |
| `--output` | Output format for the response (JSON here). |
