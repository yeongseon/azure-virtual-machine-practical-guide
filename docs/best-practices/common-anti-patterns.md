...
content_sources:
  diagrams:
  - id: best-practices-common-anti-patterns-why-this-matters
    type: flowchart
...
Frequent VM design and operations mistakes, why they persist, and how to replace them with durable patterns. The real risk is that teams usually notice weak VM design only after a deployment freeze, region capacity issue, security review, or performance incident.

**Real-world scenario**: A review board keeps seeing the same failures: public management ports, oversized VMs, underperforming disks, and restore plans that have never been tested.
...
<!-- diagram-id: best-practices-common-anti-patterns-why-this-matters -->
```mermaid
flowchart TD
    A[Anti-pattern to remediation flow] --> B[Workload classification]
...
### CLI example: common anti-patterns review
...
az vm update     --resource-group $RG     --name $VM_NAME     --set tags.reviewArea=anti-pattern-remediation tags.owner=platform-team     --output json
```
...
| `--set` | Set the review-area and owner tags (reviewArea=anti-pattern-remediation, owner=platform-team). |
| `--output` | Output format for the response (JSON here). |
