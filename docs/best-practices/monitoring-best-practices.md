...
content_sources:
  diagrams:
  - id: best-practices-monitoring-best-practices-why-this-matters
    type: flowchart
...
Signals, alert design, evidence layering, and operational dashboards. The real risk is that teams usually notice weak VM design only after a deployment freeze, region capacity issue, security review, or performance incident.

**Real-world scenario**: A service owner needs faster mean time to innocence when incidents bounce between infrastructure, OS, and application teams.
...
<!-- diagram-id: best-practices-monitoring-best-practices-why-this-matters -->
```mermaid
flowchart TD
    A[Monitoring signal pipeline] --> B[Workload classification]
...
### CLI example: monitoring best practices review
...
az vm update     --resource-group $RG     --name $VM_NAME     --set tags.reviewArea=monitoring-baseline tags.owner=platform-team     --output json
```
...
| `--set` | Set the review-area and owner tags (reviewArea=monitoring-baseline, owner=platform-team). |
| `--output` | Output format for the response (JSON here). |
