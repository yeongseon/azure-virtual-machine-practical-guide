...
Private administration, traffic segmentation, accelerated networking, and latency-aware placement. The real risk is that teams usually notice weak VM design only after a deployment freeze, region capacity issue, security review, or performance incident.

**Real-world scenario**: A hub-and-spoke estate hosts jump boxes, domain controllers, and application tiers that must stay private while allowing secure support access.
...
<!-- diagram-id: best-practices-networking-best-practices-why-this-matters -->
```mermaid
flowchart TD
    A[Private VM network pattern] --> B[Workload classification]
...
### CLI example: networking best practices review
...
az vm update     --resource-group $RG     --name $VM_NAME     --set tags.reviewArea=network-topology-hardening tags.owner=platform-team     --output json
```
...
| `--set` | Set the review-area and owner tags (reviewArea=network-topology-hardening, owner=platform-team). |
| `--output` | Output format for the response (JSON here). |
