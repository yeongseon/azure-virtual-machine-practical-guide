...
content_sources:
  diagrams:
  - id: troubleshooting-playbooks-disk-performance-issues-symptoms
    type: flowchart
...
<!-- diagram-id: troubleshooting-playbooks-disk-performance-issues-symptoms -->
```mermaid
flowchart TD
...
1. Compare disk SKU limits with VM aggregate disk bandwidth and IOPS ceilings before buying a faster disk.
2. Review host caching and move log-heavy or write-heavy paths to `None` where recommended.
3. Consider Premium SSD v2 or Ultra Disk when the workload needs elastic or very high data-plane performance.
