...
### Step 3: Apply and inspect the custom script extension

This lab validates extension deployment behavior, handler status, and failure evidence that operators need when bootstrap scripts fail.
...
| `$VM_NAME` | Target virtual machine name. |
| `--resource-group` | Scopes the command to the intended resource group. |
| `--vm-name` | Azure CLI option used to scope or shape the operation. |
